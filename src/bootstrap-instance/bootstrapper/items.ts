// @ts-ignore
import fromHTML from '@osspim/content-transformer/fromHTML'
...
async function getInstanceRootItemId(
  context: BootstrapperContext
): Promise<string> {
  const instanceId = context.instanceId
  const r = await context.callPIM({
    query: gql`
      query GET_INSTANCE_ROOT_ITEM_ID($instanceId: ID!) {
        instance {
          get(id: $instanceId) {
            rootItemId
          }
  ...
  return r.data?.instance?.get?.rootItemId || ''
}
...
export async function setItems({
  ...
  const rootItemId = await getInstanceRootItemId(context)
  ...
  async function handleItem(item: JSONItem, parentId?: string) {
    ...
    // If the item exists in OSSPIM already
    ...
    if (item.id) {
      /**
       * Store the item id for the catalogPath. Very useful if the generated
       * catalogPath is different than the one in the JSON spec
       */
      if (item.catalogPath) {
        context.itemJSONCatalogPathToIDMap.set(item.catalogPath, {
          itemId: item.id,
        ...
        itemRelations.map(async (itemRelation) => {
          if (typeof itemRelation === 'object') {
            const { itemId } = await getItemId({
              ...
              catalogPath: itemRelation.catalogPath,
              context,
              language: context.defaultLanguage.code,
              instanceId: context.instanceId,
            })

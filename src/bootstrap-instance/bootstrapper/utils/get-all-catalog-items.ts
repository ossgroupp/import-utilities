...
export async function getAllCatalogItems(
  ...
  async function getItem(path: string): Promise<JSONItem | null> {
    const itemResponse = await context.callCatalog({
    ...
    const rawData = itemResponse.data?.catalog
  ...
  async function handleItem(item: any): Promise<JSONItem> {
    ...
    async function getChildren(): Promise<JSONItem[]> {
      ...
      async function crawlChildren() {
        const pageSize = 1000
        const pageResponse = await context.callCatalog({
        ...
        const page = pageResponse.data?.catalog?.subtree
  ...
  function handleComponents(cmps?: any): Record<string, JSONComponentContent> {
    ...
    function getComponentContent(c: any): any {
      ...
      switch (c.type) {
        ...
        case 'contentChunk': {
          const chunks: any[] = []
          c.content?.chunks.forEach((catalogChunk: any[]) => {
            const chunk: Record<string, any> = {}

            catalogChunk.forEach((component) => {
  ...
  const allCatalogItems: JSONItem[] = []

  const rootItemsResponse = await context.callCatalog({
  ...
  const rootItems: { path: string }[] =
    rootItemsResponse.data?.catalog?.children || []
...
query GET_ROOT_CATALOGUE_ITEMS ($language: String!, $path: String!, $version: VersionLabel!) {
  catalog(language: $language, path: $path, version: $version) {
    children {
      path
    }
  }
...
query GET_ITEM_CHILDREN_PAGE (
  ...
  catalog(path: $path, language: $language, version: $version) {
    subtree (
      first: $pageSize
      after: $after
    ) {
